...
//by plato 04.06.12
//This is a library for working with a hex grid. The grid is made up of
//Vertex objects. Each Vertex contain a p,q coordinate pair.
...
//triangles - 6 vertices plus center, so its diameter is 2 units.

var Sexy={};          //create Sexy Scopetext

Sexy.xorigin=300;     //pixels from left to offset origin of pq
Sexy.yorigin=300;     //ditto. TODO disconnect this from canvas px 
Sexy.radius=15;       //pixels between vertices, also radius to corner of hex
Sexy.r=function(){ return Sexy.radius;};

Sexy.verts={}; //access these verts with getVert(p,q)
Sexy.hexes={}; //access these Hex objects with getHex(p_center, q_center)
Sexy.links={}; //this will hold links connecting Hex objects



//Make a hexagonal grid with radius r hexes (and radius 2r Vertexes)
//Access the grid from another js file by calling Sexy.Grid(gridRad);
//TODO pass in desired x and y container dimensions plus single hex radius 
//and we'll return a correctly shaped and sized grid with origin at center
Sexy.Grid = function(r) {
    ...
    Sexy.hexes[p.toString()]={};
        for(var q=-2*r; q<=2*r; q++) {
            ...
            //above line makes hexagon grid with rad=2r verts, rad=r hexes
...
//pq origin is offset from the top left of canvas with ctx.xorigin, ctx.yorigin
...
//note even though +p is "up" <canvas> treats +y as down, thus the odd signs
 ...
 //      Vertex
//*******************
Vertex = function(p0,q0,theta) {

    this.p=p0;    this.q=q0;    this.theta=(theta || null);
...
Vertex.prototype.pivotOut=function(theta){
//    this.theta=theta;
//    console.log(theta);
...
//    console.log(theta);
    ...
    switch(mapped) { 
        case 1: dp= 1; dq= 1; break;   //  1'ck
        case 2: dp=-1; dq= 2; break;   //  3'ck
        case 3: dp=-2; dq= 1; break;   //  5'ck
        case 4: dp=-1; dq=-1; break;   //  7'ck
        case 5: dp= 1; dq=-2; break;   //  9'ck
        case 6: dp= 2; dq=-l; break;   // 11'ck
        default: console.log("bad theta value"); break;
    };
    return getVert(this.center.p+dp, this.center.q+dq);
};
